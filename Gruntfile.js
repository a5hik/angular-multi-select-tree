  ...
  grunt.initConfig({
    ...
    concat: {
      js: {
        src: ['src/**/*main.js',
              'src/**/*.js'],
        dest: 'dist/component-<%= pkg.version %>.js'
      },
      css: {
        src: ['src/**/*.css'],
        dest: 'dist/component-<%= pkg.version %>.css'
      }
    },
    ngtemplates: {
      'as.image-slider': {
        src: 'src/**/*.html',
        dest: 'dist/component-<%= pkg.version %>.tpl.js'
      }
    },
    uglify: {
      src: {
        files: {
          'dist/component-<%= pkg.version %>.min.js': '<%= concat.js.dest %>'
        }
      }
    },
    ...
    cssmin: {
        css: {
          src: '<%= concat.css.dest %>',
          dest: 'dist/component-<%= pkg.version %>.min.css'
        }
    },
    clean: ['dist/*']
  });
